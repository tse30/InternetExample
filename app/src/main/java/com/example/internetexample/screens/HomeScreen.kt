package com.example.internetexample.screens
...
import com.example.internetexample.components.CodeCard
import com.example.internetexample.components.FilterChipGroup
import com.example.internetexample.models.Post
import com.example.internetexample.network.ApiClient
        ...
        Button(modifier = Modifier
            ...
            onClick = {
                ...
                scope.launch {
                    when (headLine) {
                        "/POST" -> {
                            jsonRepose = ApiClient.createNewPost(post).toString()
                        }

                        "/GET" -> {
                            jsonRepose = ApiClient.getAllPosts().toString()

                        }

                        "/PUT" -> {
                            // Use PUT request to Update data
                            jsonRepose = ApiClient.updatePost(
                                id = 1,
                                post = post
                            ).toString()
                        ...
                        "/DELETE" -> {
                            jsonRepose = ApiClient.deletePost(id = 2).status.toString()
